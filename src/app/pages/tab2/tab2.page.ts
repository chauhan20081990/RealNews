...
export class Tab2Page implements OnInit {
  ...
  public getNews(type: string) {
    this.http.getData('everything', this.feed.allFeed + 'page=' + this.page).subscribe(
      result => {
  ...
  public async openUserFeed() {
    ...
    modal.onDidDismiss().then((data) => {
      if (data.data) {
        this.feed.saveFeed(data.data, 'all');
        this.newsArr = [];
        this.page = 1;
